...
const GoodbyeFarewell = ['Goodbye,'];
const retourGreetings = ['Welcome back,','Nice to see you again,'];
...
const handlers = {
  'LaunchRequest': function() {
    ...
    if (this.event.session.attributes['room'] !== undefined) {
      ...
      var speechOutput = randomPhrase(welcomeGreetings) + randomPhrase(retourGreetings) + `you were playing before and got to the room called ${room['$']['name']}. Would you like to resume? `;
  ...
  'WhereAmI': function() {
    ...
    if (this.event.session.attributes['room'] === undefined) {
      ...
      speechOutput = '<audio src="https://s3.amazonaws.com/ask-soundlibrary/musical/amzn_sfx_musical_drone_intro_02.mp3"/> ' + '<audio src="https://s3.amazonaws.com/myadventurezlatoupan/bonjour_intj.mp3"/> ' + `Welcome to ${story.replace('.html','')}. Lets start your game. `;
      // How to put the randomPhrase
      // speechOutput = randomPhrase(welcomeGreetings) + `Welcome to ${story.replace('.html','')}. Lets start your game. ` + randomPhrase(welcomeGreetings);
    }
  ...
  'AMAZON.HelpIntent': function() {
    var speechOutput = 'This is an interactive adventure game. ';
    var reprompt = 'Let yourself be guided by the game, and decide where you want to go. You can interrupt me by saying; Alexa, if you already know where you want to go. Before leaving, say, stop, to save your progress. Say, ok, to continue.';
    speechOutput = speechOutput + reprompt;
    var cardTitle = 'Help.';
    var cardContent = speechOutput;
    var imageObj = undefined;
  ...
  'CompletelyExit': function() {
    ...
    var speechOutput = '<audio src="https://s3.amazonaws.com/myadventurezlatoupan/au_revoir_intj.mp3"/> '  ;
    //randomPhrase(GoodbyeFarewell) instead of the audio
    if (TableName) {
